...
climate <-
  read.csv('DailyDelhiClimateTrain.csv',
           header = T,
           sep = ",")
...
u_income <- sapply(c(0:(nrow(income) * 12 - 1)), function(x) {
  runif(1) * (income$max[floor(x) / 12 + 1] - income$min[floor(x) / 12 + 1]) +
    income$min[floor(x) / 12 + 1]
})
...
#data checking
statrep1 <- stat_rep(income[, 2])
statrep2 <- stat_rep(u_income)
sumdiff <-
  data.frame(Fields = statrep1[, 1],
             Percent_Diff = (statrep2[, 2] - statrep1[, 2]) / statrep1[, 2] * 100)
statrep1
statrep2
sumdiff
...
lm.sunspots <-
  lm(t.sunspots ~ mos - 1)
sum.lm.sunspots <- summary(lm.sunspots)

#extract the parameter values and standard errors
param.sunspots <- data.frame(
  Month = c(1:12),
...
statrep2 <- stat_rep(sim.sunspots)
sumdiff <-
  data.frame(Fields = statrep1[, 1],
             Percent_Diff = (statrep2[, 2] - statrep1[, 2]) / statrep1[, 2] * 100)
statrep1
...
sum_climate <- climate1 %>% group_by(year, month) %>%
  summarise(
    ...
    sd_humid = sd(humidity),
    avg_wind = mean(wind_speed),
    sd_wind = sd(wind_speed),
    avg_press = mean(meanpressure),
    sd_press = sd(meanpressure)
  )
...
press_check <- climate1 %>% group_by(year, month) %>%
  summarise(
    avg_press = mean(meanpressure),
    min_press = min(meanpressure),
    max_press = max(meanpressure)
  )
...
climate1 <- climate1 %>%
  mutate(is_weird = ifelse(meanpressure > 990 &
                             meanpressure < 1024, 0, 1))
...
press_sum <- climate1[-delrows, ] %>% group_by(year, month) %>%
  summarise(avg_press = mean(meanpressure),
            sd_press = sd(meanpressure))

#Replace the "weird" values of meanpressure according to the month
for (i in 1:length(delrows)) {
  curr_y = climate1$year[delrows[i]]
  curr_m = climate1$month[delrows[i]]
  sumrow = which(press_sum$year == curr_y & press_sum$month == curr_m)
  climate1$meanpressure[delrows[i]] <-
    press_sum$avg_press[sumrow] + sample(c(-1, 1), 1) * press_sum$sd_press[sumrow]
}
...
clim_sum$year <-
  as.numeric(clim_sum$year)
clim_sum$month <- as.numeric(clim_sum$month)
splom(clim_sum[, -c(1, 2, 4, 6, 8, 10)])
col <-
  colorRampPalette(c("#BB4444", "#EE9988", "#FFFFFF", "#77AADD", "#4477AA"))
corrplot(
  cor(clim_sum[, -c(1, 2, 4, 6, 8, 10)]),
  method = "color",
  col = col(200),
  addCoef.col = "black",
  type = "upper"
...
ts.climatesum <-
  list(
    ts(
      stand(clim_sum$avg_temp),
      start = 2013,
      frequency = 12
    ),
    ts(
      stand(clim_sum$avg_humid),
      start = 2013,
      frequency = 12
    ),
    ts(
      stand(clim_sum$avg_wind),
      start = 2013,
      frequency = 12
    ),
    ts(
      stand(clim_sum$avg_press),
      start = 2013,
      frequency = 12
    )
  )
...
reg_df <- data.frame(
  idx = c(1, 1, 1, 1, 2, 2, 2, 3, 3, 3, 4, 4, 4, 5, 5, 5),
...
for (i in 1:nrow(reg_df)) {
  ...
  if (reg_df$idx[i] == 1) {
    ...
    quadr2 <- c(quadr2, 0)
    adjquadr2 <- c(adjquadr2, 0)
  } else{
    x <- ts.climatesum[[(reg_df$idx[i] - 1)]]
    x2 <- x ^ 2
...
reg_df$linearr2 <- linearr2
reg_df$quadr2 <- quadr2

reg_df$adjlinearr2 <-
  adjlinearr2
reg_df$adjquadr2 <- adjquadr2
reg_df <- reg_df <- reg_df[, -c(1, 2)]
...
param.temp <-
  data.frame(
    ParVal = as.vector(summary(mod3)$coefficients[, 1]),
    ParSE = as.vector(summary(mod3)$coefficients[, 2])
  )
param.humid <- data.frame(
  Month = c(1:12),
...
currparams <-
  param.temp$ParVal + runif(48, -1, 1) * qnorm(0.975, 0, 1) * param.temp$ParSE
sim.temp <-
  currparams[1] + currparams[2] * sim.pressure + currparams[3] * sim.pressure ^
  2
...
sim.daily.temp <-
  sim.daily.humid <- sim.daily.wind <- sim.daily.pressure <- NULL
for (i in 1:4) {
  for (j in 1:12) {
    row <- i * j
    se.temp <-
      sem.temp[y] * sqrt(days$nodays[row]) * (max(clim_sum$avg_temp) - min(clim_sum$avg_temp))
    se.humid <-
      param.humid$ParSE[y] * sqrt(days$nodays[row]) * (max(clim_sum$avg_humid) -
                                                         min(clim_sum$avg_humid))
    se.wind <-
      param.wind$ParSE[y] * sqrt(days$nodays[row]) * (max(clim_sum$avg_wind) -
                                                        min(clim_sum$avg_wind))
    se.press <-
      param.pressure$ParSE[y] * sqrt(days$nodays[row]) * (max(clim_sum$avg_press) -
                                                            min(clim_sum$avg_press))
    sim.daily.temp <-
      c(sim.daily.temp,
        sim.temp[row] + runif(days$nodays[row], -1, 1) * qnorm(0.975, 0, 1) * se.temp)
    sim.daily.humid <-
      c(
        sim.daily.humid,
        sim.humid[row] + runif(days$nodays[row], -1, 1) * qnorm(0.975, 0, 1) * se.humid
      )
    sim.daily.wind <-
      c(sim.daily.wind,
        sim.wind[row] + runif(days$nodays[row], -1, 1) * qnorm(0.975, 0, 1) * se.wind)
    sim.daily.pressure <-
      c(
        sim.daily.pressure,
        sim.pressure[row] + runif(days$nodays[row], -1, 1) * qnorm(0.975, 0, 1) *
          se.press
      )
  }
}
